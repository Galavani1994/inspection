...
var ItemsComponent = /** @class */ (function () {
    ...
    ItemsComponent.prototype.insert = function () {
        if (this.update == false && this.newId == null) {
            ...
            this.database.execSQL("UPDATE itemTbl (productCharacter,productName,productId) VALUES (?,?,?)", [JSON.stringify(this.itemCharacter), this.proTitle, this.proId]).then(function (id) {
                alert('ثبت شد');
                console.log("INSERT RESULT", id);
            }, function (error) {
                console.log("INSERT ERROR", error);
    ...
    ItemsComponent.prototype.edit = function (id) {
        ...
        this.database.all("select * FROM  itemTbl WHERE id=" + id).then(function (de) {
            _this.itemCharacter = JSON.parse(de[0][1]);
